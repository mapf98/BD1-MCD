...
$('#addDEV').on('click',function(e){
	...
	$('#remove'+nextID+'').on('click',function(e){
		...
		boxMinDelete.remove();
		y++;
		//removeTotal(valueTPrev);
		//valueTPrev = $('#mon'+nextID+'').val();
		$('#Est').attr('value','');
	});
	globalIDDEV++;
});
...
$('#pagar').on('click',function(e){
	...
	console.log ('minCantidad',minCantidad);
	console.log ('precio',minPrecio);
	for (var i = minCantidad.length -1; i >=0 ; i--) {
		total = total + (minCantidad [i] * minPrecio[i]);
	};
			...
			if (response.disp != null){
				...
				console.log ('entro al ciclo');
					...
					if (parseInt(minCantidad[i]) > parseInt(response.disp[i].inv_cantidadactual)){
						...
						var estatus = 'Agotado';
						//LLEVA A SOLICITUD DE COMPRA
						// function redirect(url){
						// 		window.location.href = url;
						// 	}
						// 	setTimeout(function(){
						// 		url='/NuevaSolicitudCompra'			
						// 	},100);
						...
						console.log ('este es el estatus',estatus);
						$('#Est').attr('value',estatus);
						e.preventDefault();
						listPago = $('#listPago');
	...
	$('#tmon'+nextID+'').on('focusout',function(e){
		var mon = $(this).val();
		e.preventDefault();
		if (mon > total){
			alert ('El monto debe ser igual o menor a  '+total+' Bs');
		}
		else {
			total = total - mon;
		}
...
$('#agregarVenta').on('submit',function(e){
	...
	console.log ('estatus modificado',estatus);
			...
			if(verifyElementVal()==true){
				...
				success: function(response){
						if(response.estatus != null){
							console.log ('estatus resp', response.estatus);
							alert('La venta ha sido procesada exitosamente');
							if (response.estatus == 'Pagado'){
									function redirect(url){
									window.location.href = url;
									}
...
function presentacionprecio (presentacion,precio,cantidad,monto,addT,valueTPrev){
		...
		$.ajax({
			...
			success: function(response){
					if(response.min != null){	
						...
						total(cantidad,response,monto,addT,valueTPrev);	
					}
			}	
		});
	});	
...
function total (cantidad,response,monto,addT,valueTPrev){
...
var globalIDAC=1;
 ...
 $('#ConsultaACxmineral').on('click',function(e){
	console.log ('estoy en la funcion');
	console.log ('hizo click');
	//console.log (minPresentacionActual);
	e.preventDefault();
	$.ajax({
		url:'/ConsultarAC',
		method:'POST',
		// data:{
		// 	//minPresentacionActual:minPresentacionActual.codigo,
		// },
		success: function (response){
			if (response.compra != null){
			var nextID = globalIDAC;
				for (var i = response.compra.length-1; i >=0 ; i--) {
					e.preventDefault();
					listAC = $('#listAC');
					listAC.append(' \n\
					<div class="boxAgregarMinID animated zoomIn" id="boxAddMin" value="'+nextID+'">\n\
					...
					<div class = "col-md-8 mx-auto">\n\
						  <input type="text" class="form-control formsCRUD" id="nom'+nextID+'" value="'+response.compra[i].ac_nombre+'" disabled>\n\
					</div>\n\
					<div class="col-md-4 mx-auto">\n\
						  <button class="btn btnSave btn-block" id="pedir'+nextID+'" value="'+response.compra[i].ac_numero_rif+'">PEDIR</button>\n\
					</div>\n\
					</div>\n\
					</div>\n\
						');
		...
		$('#pedir'+nextID+'').on('click',function(e){
		e.preventDefault();
		var rif = $(this).val();	
		...
		success: function(response){
			if (response =='great'){
				alert('La solicitud fue generada exitosamente');	
			}
			else{
...
// 	// e.preventDefault();
// 	// var codigo = $(this).val();
// 	// console.log ('este es el codigo de detalle',codigo);
// 	// $.ajax({
// 	// 	url:'/DetalleVenta',
// 	// 	method:'POST',
// 	// 	data:{
...
function cambiarestatus(est){
 	$.ajax({
		url: '/VerificarSolicitud',
		method: 'POST',
		data: {
			est:est,
		},
		success: function(response){
			if (response == 'great'){
				alert ('encontro solicitud');
					function redirect(url){
					window.location.href = url;
					}
					setTimeout(function(){
					redirect("/ModificarSol");
					},100);

			//	$('#explotacion').val(response.sol[0].fk_sc_explotacion);
				$("#nombre").val(response.sol[0].ac_nombre);
				$("#explotacion").append(response.sol[0].fk_sc_explotacion);

				
			}
			else{
				alert('no encontro solicitud')
			}
	...
	$('#mod').on('click',function(e){
		e.preventDefault();
		var codigo =$(this).val();
		var estatus = $('#est').children(":selected").val();
		console.log ('este es el estatus', estatus);
		$.ajax({
		...
		success: function(response){
			if (response =='great'){
				alert('La solicitud fue modificada exitosamente');	
			}
			else{
				alert('No se pudo modificar la solicitud');
			}	//cierre success	
		}
	});//cierre ajax
