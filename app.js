...
// jsreport.use(require('jsreport-core')());
...
app.get("/Explotaciones",function(req,res){
  if(userJSON.usuario != "none"){
    res.render('explotaciones',{user: userJSON,minPresentacionActual:minPresentacionActual});
  }else{
    res.redirect('login');
  }
});
...
app.post("/ConsultarInv",function(req,res){
  ...
  minPresentacionActual.codigo = minPresentacion;
  console.log ('ESTE ES PRESENTACION ACTUAL',minPresentacionActual);
...
app.get("/NuevaSolicitudCompra",function(req,res){
  if(userJSON.usuario != "none"){
    res.render('consultacompraaliados.ejs',{user: userJSON, minPresentacionActual:minPresentacionActual});
  }else{
    res.redirect('login');
  }
});

app.post("/ConsultarAC",function(req,res){
  var pres = req.body.presentacion;
  if (userJSON.usuario !="none"){
    client.query('select mt.met_codigo, nm.nom_codigo from min_pre left join min_no_metalico nm on fk_mp_nometalico = nm.nom_codigo left join min_metalico mt on fk_mp_metalico=mt.met_codigo where mp_codigo = $1',
      [parseInt(minPresentacionActual.codigo)],(err,result)=>{
      if (err){
        console.log(err.stack);
        res.send('failed'); 
      }
      else if(result.rows[0].met_codigo != null){   
        var codigo = result.rows; 
              client.query('select ac.ac_numero_rif, ac.ac_nombre from aliado_comercial ac, ali_mm amm, min_min mm where amm.fk_amm_aliado = ac.ac_numero_rif and amm.fk_amm_mm = mm.mm_codigo and (mm.fk_mm_1metalico = $1 or mm.fk_mm_2metalico = $2)',
            [codigo[0].met_codigo,codigo[0].met_codigo],(err,result)=>{  
              if (err){
              console.log(err.stack);
              res.send('failed'); 
              }
                ...
                console.log (result.rows);
                var compra = result.rows;
                res.send ({compra:compra});
              }
            });
      }else{
        var codigo = result.rows;
                client.query('select ac.ac_numero_rif, ac.ac_nombre from aliado_comercial ac, ali_mm amm, min_min mm where amm.fk_amm_aliado = ac.ac_numero_rif and amm.fk_amm_mm = mm.mm_codigo and (mm.fk_mm_1nometalico = $1 or mm.fk_mm_2nometalico = $2)',
            [codigo[0].nom_codigo,codigo[0].nom_codigo],(err,result)=>{  
              if (err){
              console.log(err.stack);
              res.send('failed'); 
              }
...
app.post("/GenerarSolicitud",function(req,res){
  console.log ('entro a generar solicitud');
  ...
  if (userJSON.usuario !="none"){
    console.log ('este es el rif', rif);
    client.query('INSERT INTO solicitud_compra(sc_fechaemision, sc_costototal, fk_sc_aliado, fk_sc_estatus, fk_sc_explotacion) VALUES ((select now ()), $1, $2, (select est_codigo from estatus where est_nombre =$3), $4)',
      [costototal,rif,estatus,explotacion],(err,result)=>{
...
app.get("/ModificarSol",function(req,res){
  if(userJSON.usuario != "none"){
    res.render('Solicitudmodificar.ejs',{solicitud:solicitud, user: userJSON});
  }else{
    res.redirect('login');
  }
});
...
app.post("/VerificarSolicitud",function(req,res){
  var fy='yyyy';
  var fm='mm';
  var fd= 'dd';
  var codigo= req.body.est;

  if(userJSON.usuario != "none"){
    client.query('SELECT S.sc_codigo, to_char(S.sc_fechaemision,$1) AS year,to_char(S.sc_fechaemision,$2) AS month, to_char(S.sc_fechaemision,$3) AS day, S.sc_costototal, S.fk_sc_aliado, AC.ac_nombre,S.fk_sc_estatus, AC.ac_numero_rif FROM solicitud_compra AS S, aliado_comercial AS AC WHERE S.fk_sc_aliado = AC.ac_numero_rif and sc_codigo = $4',
      [fy,fm,fd,codigo],(err,result)=>{
      if (err) {
        console.log(err.stack);
        res.send('failed'); 
      }else if(result.rows[0] != null){
      ...
      solicitud.aliado =result.rows[0].ac_nombre;
      solicitud.fecha = (result.rows[0].day+-+result.rows[0].month+-+result.rows[0].year);
      solicitud.rif =result.rows[0].ac_numero_rif;
      solicitud.estatus = result.rows[0].fk_sc_estatus;
      solicitud.costo = result.rows[0].sc_costototal;
      solicitud.codigo = result.rows[0].sc_codigo;
...
app.post("/ModificarSolicitud",function(req,res){
   var cod = req.body.codigo;
   var estatus = req.body.estatus;
   console.log(cod);
   console.log(estatus);
  if(userJSON.usuario != "none"){
    client.query('update solicitud_compra set fk_sc_estatus = $1 where sc_codigo=$2',
    [estatus,cod],(err,result)=>{
    console.log ('hizo la modificacion');
    if (err) {
        console.log(err.stack);
      ...
      if (err) {
        ...
        for (var i = devMonto.length - 1; i >= 0; i--) {
            ...
            [codigoventa[0].ven_codigo,devMonto[i], minCantidad[i], minPresentacion[i]],(err,resultM)=>{
                     console.log('el estatus es 2',estatus);
              if (err) {
                console.log(err.stack);
                res.send('failed'); 
              }
                        ...
                        if (tipopago[i]=="Transf"){
                          ...
                          if(err){
                           ...
                           client.query('INSERT INTO public.ven_tip(vt_fecha, vt_monto, fk_vt_tptransferencia, fk_vt_venta) VALUES ((Select now()),$1,$2,$3)', 
                            ...
                            });//cierre query venntip
                          } 
                        });//cierre query tp_transf
                      }// cierre if transf
              ...
              } // cierre else tipo pago


          });//cierre query detalle venta
          }
        }//cierre else if 28
        console.log('este es ',estatus);
        res.send({estatus:estatus});
      });// cierre query venta
  }else{
    res.redirect('login');
